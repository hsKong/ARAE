...
def load_models_decode(load_path):
    model_args = json.load(open("{}/args.json".format(load_path), "r"))
    word2idx = json.load(open("{}/vocab.json".format(load_path), "r"))
    idx2word = {v: k for k, v in word2idx.items()}
    ...
    return model_args, idx2word, autoencoder, gan_gen, gan_disc, word2idx
    #word2idx추가(kong)
    
def load_models_autoencoder(load_path):
    model_args = json.load(open("{}/args.json".format(load_path), "r"))
    word2idx = json.load(open("{}/vocab.json".format(load_path), "r"))
    idx2word = {v: k for k, v in word2idx.items()}
    ...
    return model_args, idx2word, autoencoder,word2idx
    #word2idx추가(kong)
